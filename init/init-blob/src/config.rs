...
impl Config {
    ...
    ///
    /// Populates `overlay` with the init binary and config files, and
    /// appends the init kernel cmdline arg to `payload`.
    ///
    ...
    pub fn apply(
        ...
        #[cfg_attr(
            feature = "ffi",
            ffier(foreign = krun_via_cdylib_weak, c_name = "KrunFsOverlay")
        )]
        overlay: &mut FsOverlay,
        #[cfg_attr(
            feature = "ffi",
            ffier(foreign = krun_via_cdylib_weak, c_name = "KrunPayload")
        )]
        payload: &mut Payload,
    ) -> Result<(), ApplyError> {
        krun::require(
...
impl Builder {
    ...
    pub fn set_root_disk_remount(
        ...
        self.inner
            .process
            .env
            .retain(|e| !e.starts_with("KRUN_BLOCK_ROOT_DEVICE="));
        self.inner
            .process
            .env
