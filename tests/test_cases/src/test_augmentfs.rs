...
mod host {
    ...
    impl Test for TestAugmentFs {
        fn start_vm(self: Box<Self>, test_setup: TestSetup) -> anyhow::Result<()> {
            ...
            overlay
                .add_file("guest-agent", guest_agent_bytes, 0o100_755, true)
                .map_err(|e| anyhow::anyhow!("overlay.add_file guest-agent: {e:?}"))?;
            // Persistent marker file.
            overlay
                .add_file("marker.txt", marker, 0o100_644, false)
                .map_err(|e| anyhow::anyhow!("overlay.add_file marker: {e:?}"))?;
            // Deterministic 8 KiB payload for range-read tests.
            overlay
                .add_file("testdata.bin", test_payload, 0o100_444, false)
                .map_err(|e| anyhow::anyhow!("overlay.add_file testdata: {e:?}"))?;
            // Nested path test: etc/nested/deep.txt
            overlay
                .add_dir("etc", 0o040_755)
                .map_err(|e| anyhow::anyhow!("overlay.add_dir etc: {e:?}"))?;
            overlay
                .add_dir("etc/nested", 0o040_755)
                .map_err(|e| anyhow::anyhow!("overlay.add_dir etc/nested: {e:?}"))?;
            overlay
                .add_file("etc/nested/deep.txt", nested_content, 0o100_644, false)
                .map_err(|e| anyhow::anyhow!("overlay.add_file deep.txt: {e:?}"))?;
