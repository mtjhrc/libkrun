...
type ExtraDevicesFn = Box<dyn FnOnce(&mut krun::MmioDeviceManager<'_>) -> anyhow::Result<()>>;
...
pub fn require_freebsd_symbols() -> Result<(), libloading::Error> {
    ...
    krun::require(
        ...
        &[
            ...
            krun::Symbol::KrunBlockDeviceSetReadOnly,
...
fn do_setup_and_enter(
    ...
    let vmm = krun::VmmBuilder::new()
        ...
        .add_serial_console(Some(serial_read_fd.as_fd()), Some(stdout.as_fd()))
        .map_err(|e| anyhow::anyhow!("add_serial_console: {e:?}"))?
        .payload(payload)
        .devices(devices)
        .build()
