...
/// This allows guest connections to gateway:port to be forwarded to host localhost:port.
fn expose_port(control_socket_path: &std::path::Path, port: u16) -> std::io::Result<()> {
    ...
    if !response.contains("200") {
        return Err(std::io::Error::new(
            ...
            format!("gvproxy expose failed: {}", response),
        ));
    }
    Ok(())
}
...
pub fn setup_backend(ctx: u32, test_setup: &TestSetup) -> anyhow::Result<()> {
    ...
    // Wait for control socket too
    anyhow::ensure!(
        wait_for_socket(&control_socket_path, 5000),
        "gvproxy failed to create control socket"
    );

    // Expose port 8002 so guest can connect to host TCP server
    const GVPROXY_PORT: u16 = 8002;
    expose_port(&control_socket_path, GVPROXY_PORT)?;
