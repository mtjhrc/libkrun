...
impl Payload {
    ...
    pub fn load_krunfw_tee(
        ...
        #[cfg(feature = "tee")]
        {
            ...
            let bundle = load_kernel_bundle(lib)?;
            #[cfg(feature = "tdx")]
            let (qboot_bundle, initrd_bundle) = if firmware_path.is_some() {
    ...
    pub fn load_external(
        ...
        let (initramfs_path, initramfs_size) =
            if let Some(initrd) = initrd_path.filter(|s| !s.is_empty()) {
...
) -> Result<crate::vmm::vmm_config::kernel_bundle::KernelBundle, Error> {
...
fn load_initrd_bundle(
    lib: &libloading::Library,
) -> Result<crate::vmm::vmm_config::kernel_bundle::InitrdBundle, Error> {
...
impl From<KernelFormat> for crate::vmm::vmm_config::external_kernel::KernelFormat {
    fn from(format: KernelFormat) -> Self {
        match format {
            KernelFormat::Elf => Self::Elf,
            KernelFormat::Raw => Self::Raw,
            KernelFormat::PeGz => Self::PeGz,
            KernelFormat::ImageBz2 => Self::ImageBz2,
            KernelFormat::ImageGz => Self::ImageGz,
            KernelFormat::ImageZstd => Self::ImageZstd,
        }
    }
}
