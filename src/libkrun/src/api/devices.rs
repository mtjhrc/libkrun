...
use devices::virtio::{PortDescription, VirtioDevice, VmmExitObserver, port_io};
...
pub use crate::attach::{DeviceRequirements, ResolvedShmRegion};
use crate::attach::AttachContext as InnerAttachContext;

use super::error::{DetailedError, Error};

// ---------------------------------------------------------------------------
// AttachContext — typed wrapper around the internal attach scaffold
// ---------------------------------------------------------------------------

pub struct AttachContext<'a>(InnerAttachContext<'a>);
...
impl<'a> AttachContext<'a> {
    pub(crate) fn new_mmio(
        ...
        Self(InnerAttachContext::new_mmio(
            vmm,
            event_manager,
            shm_manager,
            intc,
    ...
    pub fn register(
        ...
        self.0
            .register_mmio_device(id, device)
    ...
    pub fn push_exit_observer(&mut self, observer: Arc<Mutex<dyn VmmExitObserver>>) {
        self.0.push_exit_observer(observer);
    }
    ...
    pub fn resolved_gpu_shm_region(&self) -> Option<ResolvedShmRegion> {
        self.0.resolved_gpu_shm_region()
    }
    ...
    pub fn append_kernel_cmdline(&mut self, s: &str) {
        self.0.append_kernel_cmdline(s);
    }
}
