...
impl Default for NetworkConfig {
    ...
    fn default() -> Self {
        NetworkConfig::ModeTSI(Default::default())
    }
}
...
impl ContextConfig {
    ...
    fn set_port_map(&mut self, new_port_map: HashMap<u16, u16>) -> Result<(), ()> {
        match &mut self.net_cfg {
            NetworkConfig::ModeNone => Err(()),
            NetworkConfig::ModeTSI(tsi_config) => {
...
#[allow(clippy::missing_safety_doc)]
#[no_mangle]
pub unsafe extern "C" fn krun_set_net_mode(ctx_id: u32, net_mode: u32) -> i32 {
    match CTX_MAP.lock().unwrap().entry(ctx_id) {
        Entry::Occupied(mut ctx_cfg) => {
            let cfg = ctx_cfg.get_mut();
            let net_cfg = match net_mode {
...
pub unsafe extern "C" fn krun_set_port_map(ctx_id: u32, c_port_map: *const *const c_char) -> i32 {
    ...
    match CTX_MAP.lock().unwrap().entry(ctx_id) {
        Entry::Occupied(mut ctx_cfg) => {
            let cfg = ctx_cfg.get_mut();
            if let Err(()) = cfg.set_port_map(port_map) {
                return -libc::ENOTSUP;
            }
        }
        Entry::Vacant(_) => return -libc::ENOENT,
...
pub extern "C" fn krun_start_enter(ctx_id: u32) -> i32 {
    ...
    match ctx_cfg.net_cfg {
        NetworkConfig::ModeNone => (),
        NetworkConfig::ModeTSI(tsi_cfg) => {
