...
#[cfg(any(feature = "tee", all(target_arch = "aarch64", target_os = "macos")))]
...
// `build_microvm()` and its per-device attach helpers relocated to
// `libkrun::boot` so they can attach devices through the internal
// `AttachContext` in `libkrun::attach`. See that module for the v1 boot
// recipe.
...
// `attach_fs_devices()` and the console-port autoconfiguration helpers
// relocated to `libkrun::boot` along with `build_microvm()`.
...
// `create_explicit_ports()`, `attach_console_devices()`, and the other
// per-device-kind attach functions (net/vsock/balloon/block/rng/vhost-user
// /gpu/input) relocated to `libkrun::boot` along with `build_microvm()`.
